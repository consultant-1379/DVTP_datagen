...
implements EcdsEventsStreamUtilityInterface{
	...
	private byte[] dataUsageEvent(String[] elements, int lineNo, String file,int eventId, String version) 
    {
        ...
        try
        {
            ...
            /*int currentIndex=30;
            bearerCount=Integer.parseInt(elements[29]);
            for(int i=0;i<bearerCount;i++){
            setParam(p, elements, "BEARER_USAGE_INFO_EPS_BEARER_ID", currentIndex++, lineNo, file, eventId);
			setParam(p, elements, "BEARER_USAGE_INFO_BEARER_CAUSE", currentIndex++, lineNo, file, eventId);
			setParam(p, elements, "BEARER_USAGE_INFO_BEARER_UL_PACKETS", currentIndex++, lineNo, file, eventId);
			setParam(p, elements, "BEARER_USAGE_INFO_BEARER_DL_PACKETS", currentIndex++, lineNo, file, eventId);
			setParam(p, elements, "BEARER_USAGE_INFO_BEARER_UL_BYTES", currentIndex++, lineNo, file, eventId);
			setParam(p, elements, "BEARER_USAGE_INFO_BEARER_DL_BYTES", currentIndex++, lineNo, file, eventId);
			/*setParam(p, elements, "SESSION_USAGE_INFO_RATING_GROUP", 35, lineNo, file, eventId);
			setParam(p, elements, "SESSION_USAGE_INFO_SERVICE_IDENTIFIER", 36, lineNo, file, eventId);
			setParam(p, elements, "SESSION_USAGE_INFO_URI_NAME", 37, lineNo, file, eventId);
			setParam(p, elements, "SESSION_USAGE_INFO_URI_ID", 38, lineNo, file, eventId);
			setParam(p, elements, "SESSION_USAGE_INFO_SERVICE_UL_BYTES", 39, lineNo, file, eventId);
			setParam(p, elements, "SESSION_USAGE_INFO_SERVICE_DL_BYTES", 40, lineNo, file, eventId);
            }
            sessionCount=Integer.parseInt(elements[currentIndex]);
            setParam(p, elements, "SESSION_USAGE_COUNT", currentIndex++, lineNo, file, eventId);*/
        }
